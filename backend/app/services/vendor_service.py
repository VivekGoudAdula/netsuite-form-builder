...
from app.services.netsuite_service import fetch_vendors_from_netsuite
...
def _to_api_row(row: Dict[str, str]) -> Dict[str, Any]:
    ...
    return {
        ...
        "subsidiaryId": str(row.get("subsidiaryId") or ""),
...
async def fetch_vendors_live(*, force_refresh: bool = False) -> List[Dict[str, str]]:
    now = time.time()
    cached = _cache.get("rows")
    if (
        not force_refresh
    ...
    rows = await fetch_vendors_from_netsuite()
    if rows:
        _cache["rows"] = rows
        _cache["at"] = now
        logger.info("Vendor cache refreshed from NetSuite count=%s", len(rows))
        return rows

    stale = _stale_rows()
    if stale is not None:
        logger.warning(
            "Vendor fetch failed — serving stale in-memory cache count=%s",
            len(stale),
        )
        return stale
...
async def _log_fetch(
    ...
    *,
    ...
    try:
        ...
        logger.exception("Failed to write FETCH_VENDORS activity log")


async def get_vendors_live(
    ...
    *,
    ...
    try:
        rows = _filter_rows(await fetch_vendors_live(), search=search)
        page = max(1, page)
        ...
        logger.exception("Vendor live fetch failed")
        await _log_fetch(db, user_id=user_id, response_count=0, success=False)
        return {
            "success": False,
            "message": "Unable to fetch vendor data",
            "count": 0,
            "data": [],
        }
...
async def search_vendors(
    ...
    *,
    ...
    rows = _filter_rows(await fetch_vendors_live(), search=q)
    total = len(rows)
    start = (page - 1) * limit
    page_rows = rows[start : start + limit]
    return {
...
async def get_vendor_by_internal_id(
    ...
    if not iid:
        return None
    for row in await fetch_vendors_live():
