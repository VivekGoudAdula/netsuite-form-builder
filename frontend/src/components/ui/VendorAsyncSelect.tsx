...
export function VendorAsyncSelect({
  ...
  if (preview) {
    return (
      <div
        className={cn(
          'h-9 border border-ns-border rounded-ns-md px-3 flex items-center bg-white text-[12px] text-ns-text-muted',
          className,
        )}
      >
        — Vendor —
    ...
    open && !disabled && panelRect
      ? createPortal(
          <ul
            ref={panelRef}
            role="listbox"
            className="max-h-72 overflow-y-auto overflow-x-hidden rounded-ns-md border border-ns-border bg-white text-[12px] shadow-2xl ring-1 ring-black/5"
            style={{
              position: 'fixed',
              top: panelRect.top,
              left: panelRect.left,
