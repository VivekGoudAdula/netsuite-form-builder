...
export default function FormsPage() {
  ...
  const relevantUsers = users.filter(u => u.companyId === activeForm?.customerId);
      ...
      <div className="space-y-8">
        ...
        <div className="flex justify-between items-end">
          <div>
            <div className="flex items-center gap-2 text-ns-blue mb-1">
              <Layout size={16} />
              <span className="text-[10px] font-bold uppercase tracking-[0.2em]">Transaction Layout Repository</span>
            </div>
            <h1 className="text-3xl font-bold text-ns-text">Manage Blueprints</h1>
            <p className="text-sm text-ns-text-muted mt-1">Configure and assign NetSuite-compatible schemas to client entities.</p>
          </div>
        ...
        <div className="bg-white p-6 rounded-sm border border-ns-border ns-panel-shadow flex items-center justify-between gap-6">
          <div className="relative flex-1 max-w-md">
            ...
            <Input 
              placeholder="Filter by blueprint name or transaction type..." 
              className="pl-9 h-10"
              value={searchTerm}
        ...
        <Table>
          ...
          <TBody>
            ...
            {filteredForms.length === 0 && (
              <TR>
                <TD colSpan={5} className="py-24 text-center bg-white space-y-4">
                  <div className="opacity-40 flex flex-col items-center">
                    ...
                    <p className="text-[10px] mt-2">Zero matching blueprints found in your directory.</p>
                  </div>
          ...
          >
            <div className="space-y-6">
              <div className="p-4 bg-ns-navy rounded-sm text-white">
                <p className="text-[10px] font-bold uppercase tracking-widest opacity-60 mb-1">Target Blueprint</p>
                <h3 className="text-lg font-bold">{activeForm.name}</h3>
                <p className="text-xs opacity-80 mt-1 italic">Assign authorized corporate staff from {companies.find(c => c.id === activeForm.customerId)?.name}.</p>
              </div>
                  ...
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-ns-text-muted font-medium uppercase tracking-tight">{user.jobTitle || 'Corporate Staff'}</p>
                      </div>
                    </div>
                    {selectedUserIds.includes(user.id) && (
