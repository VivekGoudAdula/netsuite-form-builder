...
import { formatHsnOptionLabel } from '../../lib/netsuiteMasterData';
import { Hash, RefreshCw, Search, Zap } from 'lucide-react';
...
export default function HSNManagementPage() {
  ...
  }, [debouncedSearch, fetchHSN, hsnListLimit]);
  ...
  const handleRefreshNetSuite = async () => {
    ...
    setError(null);
    try {
      const res = await syncHSN();
      if (res) {
        setMessage(`Loaded ${res.fetched} HSN codes live from NetSuite (not stored in MongoDB).`);
      }
    } catch (e: any) {
  ...
  const totalPages = Math.max(1, Math.ceil(hsnListCount / hsnListLimit));
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
          <h1 className="text-2xl font-bold text-ns-navy flex items-center gap-2">
            <Hash className="text-ns-blue" />
            HSN Codes
          </h1>
          <p className="text-ns-text-muted text-sm mt-1 max-w-xl">
            Live NetSuite RESTlet data. MongoDB stores form metadata only — not HSN records.
          </p>
        </div>
        <div className="flex gap-2">
          ...
          >
            <RefreshCw size={16} className={cn(loadingHSN && 'animate-spin')} />
            Reload list
          </button>
          <button
            type="button"
      ...
      <div className="bg-white border border-ns-border rounded-sm ns-panel-shadow overflow-hidden">
        <div className="p-4 border-b border-ns-border flex flex-col sm:flex-row gap-3 sm:items-center sm:justify-between">
          <div className="relative max-w-md w-full">
            ...
            <input
              type="text"
              placeholder="Filter by name, hsncode, or internal ID…"
              value={search}
          ...
          <span className="text-[11px] text-ns-text-muted font-semibold">
            {hsnListCount.toLocaleString()} from NetSuite
          </span>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs">
            <thead className="bg-gray-50 border-b border-ns-border text-ns-text-muted uppercase tracking-wider">
              <tr>
                <th className="px-4 py-3 font-bold min-w-[280px]">HSN</th>
                <th className="px-4 py-3 font-bold">HSN Code</th>
            ...
            <tbody className="divide-y divide-ns-border">
              {loadingHSN && hsnCodes.length === 0 ? (
                ...
                <tr>
                  <td colSpan={3} className="px-4 py-8 text-center text-ns-text-muted">
                    No HSN codes returned. Check NetSuite OAuth / RESTlet, then Refresh from NetSuite.
                  </td>
                </tr>
              ) : (
                hsnCodes.map(row => (
                  <tr key={row._id} className="hover:bg-ns-gray-bg/50">
                    ...
                    <td className="px-4 py-2 text-ns-text-muted">{row.hsncode || '—'}</td>
        ...
        <div className="p-3 border-t border-ns-border flex flex-wrap items-center justify-between gap-2 text-[11px]">
          <span className="text-ns-text-muted">
            Page {hsnListPage} of {totalPages}
          </span>
          <div className="flex gap-2">
            <button
              type="button"
              disabled={hsnListPage <= 1 || loadingHSN}
              onClick={() =>
                fetchHSN({
                  page: hsnListPage - 1,
                  limit: hsnListLimit,
            ...
            <button
              type="button"
              disabled={hsnListPage >= totalPages || loadingHSN}
              onClick={() =>
                fetchHSN({
                  page: hsnListPage + 1,
                  limit: hsnListLimit,
